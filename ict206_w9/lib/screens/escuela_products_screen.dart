...
import '../models/escuela_product.dart';

class EscuelaProductsScreen extends StatefulWidget {
  const EscuelaProductsScreen({super.key});

  @override
  State<EscuelaProductsScreen> createState() => _EscuelaProductsScreenState();
}

class _EscuelaProductsScreenState extends State<EscuelaProductsScreen> {
  late Future<List<EscuelaProduct>> _productsFuture;
  List<EscuelaProduct> _allProducts = [];
  List<EscuelaProduct> _filteredProducts = [];
  String _searchQuery = '';
  ...
  Future<List<EscuelaProduct>> fetchProducts() async {
    final response = await http.get(Uri.parse('https://api.escuelajs.co/api/v1/products'));
    if (response.statusCode == 200) {
      List jsonResponse = json.decode(response.body);
      _allProducts = jsonResponse.map((product) => EscuelaProduct.fromJson(product)).toList();
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: FutureBuilder<List<EscuelaProduct>>(
                ...
                if (snapshot.hasData) {
                  return ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      EscuelaProduct product = _filteredProducts[index];
  ...
  void _showProductDialog(BuildContext context, EscuelaProduct product) {
    showDialog(
      context: context,
